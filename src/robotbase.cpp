...
int robotbase_init(void)
{
	...
	controller.setCleanMode(POWER_ACTIVE);
	g_send_stream_mutex.lock();
	memcpy(buf, controller.send_stream, sizeof(uint8_t) * SEND_LEN);
	g_send_stream_mutex.unlock();
	uint8_t crc;
	crc = calc_buf_crc8(buf, SEND_LEN - 3);
	controller.setSendData(SEND_LEN - 3, crc);
...
void debug_received_stream()
{
	...
	for (int i = 0; i < RECEI_LEN; i++)
		printf("%02x ", controller.receive_stream[i]);
	printf("\n");
}
...
void robotbase_deinit(void)
{
	...
	if (is_robotbase_init) {
		...
		controller.setSendData(CTL_BUZZER, 0x00);
		gyro.setOff();
		cs_disable_motors();
...
void robotbase_reset_send_stream(void)
{
	...
	for (int i = 0; i < SEND_LEN; i++) {
		if (i != CTL_LED_GREEN)
			controller.setSendData(i, 0x00);
		else
			controller.setSendData(i, 0x64);
	}
	controller.setSendData(0, 0xaa);
	controller.setSendData(1, 0x55);
	controller.setSendData(SEND_LEN - 2, 0xcc);
	controller.setSendData(SEND_LEN - 1, 0x33);
}
	...
	while (ros::ok() && (!robotbase_thread_stop)) {
		...
		if (r_crc == c_crc){
			if (receiData[wh_len - 1] == t2 && receiData[wh_len - 2] == t1) {
				for (j = 0; j < wht_len; j++) {
					controller.receive_stream[j + 2] = receiData[j];
				}
...
void *robotbase_routine(void*)
{
	...
	while (ros::ok() && !robotbase_thread_stop)
	{
		...
		lw_speed = (controller.receive_stream[REC_LW_S_H] << 8) | controller.receive_stream[REC_LW_S_L];
		rw_speed = (controller.receive_stream[REC_RW_S_H] << 8) | controller.receive_stream[REC_RW_S_L];
		...
		sensor.angle = -(float)((int16_t)((controller.receive_stream[REC_ANGLE_H] << 8) | controller.receive_stream[REC_ANGLE_L])) / 100.0;//ros angle * -1
		sensor.angle -= robot::instance()->offsetAngle();
		gyro.setAngle(sensor.angle);
		sensor.angle_v = -(float)((int16_t)((controller.receive_stream[REC_ANGLE_V_H] << 8) | controller.receive_stream[REC_ANGLE_V_L])) / 100.0;//ros angle * -1
		gyro.setAngleV(sensor.angle_v);

		sensor.lw_crt = (((controller.receive_stream[REC_LW_C_H] << 8) | controller.receive_stream[REC_LW_C_L]) & 0x7fff) * 1.622;
		sensor.rw_crt = (((controller.receive_stream[REC_RW_C_H] << 8) | controller.receive_stream[REC_RW_C_L]) & 0x7fff) * 1.622;

		sensor.left_wall = ((controller.receive_stream[REC_L_WALL_H] << 8)| controller.receive_stream[REC_L_WALL_L]);

		sensor.l_obs = ((controller.receive_stream[REC_L_OBS_H] << 8) | controller.receive_stream[REC_L_OBS_L]) - obs.get_left_baseline();
		sensor.f_obs = ((controller.receive_stream[REC_F_OBS_H] << 8) | controller.receive_stream[REC_F_OBS_L]) - obs.get_front_baseline();
		sensor.r_obs = ((controller.receive_stream[REC_R_OBS_H] << 8) | controller.receive_stream[REC_R_OBS_L]) - obs.get_right_baseline();

#if __ROBOT_X900

		sensor.right_wall = ((controller.receive_stream[REC_R_WALL_H]<<8)|controller.receive_stream[REC_R_WALL_L]);

		sensor.lbumper = (controller.receive_stream[REC_BUMPER] & 0xf0) ? true : false;
		sensor.rbumper = (controller.receive_stream[REC_BUMPER] & 0x0f) ? true : false;
		...
		sensor.ir_ctrl = controller.receive_stream[REC_REMOTE_IR];
		if (sensor.ir_ctrl > 0)
			remote.set(sensor.ir_ctrl);

		sensor.c_stub = (controller.receive_stream[REC_CHARGE_STUB_4] << 24 ) | (controller.receive_stream[REC_CHARGE_STUB_3] << 16)
			| (controller.receive_stream[REC_CHARGE_STUB_2] << 8) | controller.receive_stream[REC_CHARGE_STUB_1];
		c_rcon.set_status(c_rcon.get_status() | sensor.c_stub);

		sensor.visual_wall = (controller.receive_stream[REC_VISUAL_WALL_H] << 8)| controller.receive_stream[REC_VISUAL_WALL_L];

		sensor.key = controller.receive_stream[REC_KEY];
		key.eliminate_jitter(sensor.key);

		sensor.c_s = controller.receive_stream[REC_CHARGE_STATE];

		sensor.w_tank = (controller.receive_stream[REC_WATER_TANK]>0)?true:false;

		sensor.batv = (controller.receive_stream[REC_BAT_V]);

		if(((controller.receive_stream[REC_L_CLIFF_H] << 8) | controller.receive_stream[REC_L_CLIFF_L]) < CLIFF_LIMIT)
		{
			if(last_lcliff > CLIFF_LIMIT)
				last_lcliff = ((controller.receive_stream[REC_L_CLIFF_H] << 8) | controller.receive_stream[REC_L_CLIFF_L]);
			else
				sensor.lcliff = last_lcliff = ((controller.receive_stream[REC_L_CLIFF_H] << 8) | controller.receive_stream[REC_L_CLIFF_L]);
		}
		else
			sensor.lcliff = last_lcliff = ((controller.receive_stream[REC_L_CLIFF_H] << 8) | controller.receive_stream[REC_L_CLIFF_L]);
		if(((controller.receive_stream[REC_F_CLIFF_H] << 8) | controller.receive_stream[REC_F_CLIFF_L]) < CLIFF_LIMIT)
		{
			if(last_fcliff > CLIFF_LIMIT)
				last_fcliff = ((controller.receive_stream[REC_F_CLIFF_H] << 8) | controller.receive_stream[REC_F_CLIFF_L]);
			else
				sensor.fcliff = last_fcliff = ((controller.receive_stream[REC_F_CLIFF_H] << 8) | controller.receive_stream[REC_F_CLIFF_L]);
		}
		else
			sensor.fcliff = last_fcliff = ((controller.receive_stream[REC_F_CLIFF_H] << 8) | controller.receive_stream[REC_F_CLIFF_L]);
		if(((controller.receive_stream[REC_R_CLIFF_H] << 8) | controller.receive_stream[REC_R_CLIFF_L]) < CLIFF_LIMIT)
		{
			if(last_rcliff > CLIFF_LIMIT)
				last_rcliff = ((controller.receive_stream[REC_R_CLIFF_H] << 8) | controller.receive_stream[REC_R_CLIFF_L]);
			else
				sensor.rcliff = last_rcliff = ((controller.receive_stream[REC_R_CLIFF_H] << 8) | controller.receive_stream[REC_R_CLIFF_L]);
		}
		else
			sensor.rcliff = last_rcliff = ((controller.receive_stream[REC_R_CLIFF_H] << 8) | controller.receive_stream[REC_R_CLIFF_L]);

		sensor.vacuum_selfcheck_status = (controller.receive_stream[REC_CL_OC] & 0x30);
		sensor.lbrush_oc = (controller.receive_stream[REC_CL_OC] & 0x08) ? true : false;		// left brush over current
		sensor.mbrush_oc = (controller.receive_stream[REC_CL_OC] & 0x04) ? true : false;		// main brush over current
		sensor.rbrush_oc = (controller.receive_stream[REC_CL_OC] & 0x02) ? true : false;		// right brush over current
		sensor.vcum_oc = (controller.receive_stream[REC_CL_OC] & 0x01) ? true : false;		// vaccum over current

		sensor.gyro_dymc = controller.receive_stream[REC_GYRO_DYMC];

		sensor.omni_wheel = (controller.receive_stream[REC_OMNI_W_H]<<8)|controller.receive_stream[REC_OMNI_W_L];

		if (last_x_acc == -1000)
			sensor.x_acc = (controller.receive_stream[REC_XACC_H]<<8)|controller.receive_stream[REC_XACC_L];//in mG
		else
			sensor.x_acc = ((controller.receive_stream[REC_XACC_H]<<8)|controller.receive_stream[REC_XACC_L] + last_x_acc) / 2;//in mG
		if (last_y_acc == -1000)
			sensor.y_acc = (controller.receive_stream[REC_YACC_H]<<8)|controller.receive_stream[REC_YACC_L];//in mG
		else
			sensor.y_acc = ((controller.receive_stream[REC_YACC_H]<<8)|controller.receive_stream[REC_YACC_L] + last_y_acc) / 2;//in mG
		if (last_z_acc == -1000)
			sensor.z_acc = (controller.receive_stream[REC_ZACC_H]<<8)|controller.receive_stream[REC_ZACC_L];//in mG
		else
			sensor.z_acc = ((controller.receive_stream[REC_ZACC_H]<<8)|controller.receive_stream[REC_ZACC_L] + last_z_acc) / 2;//in mG
#if GYRO_FRONT_X_POS
		...
		sensor.plan = controller.receive_stream[REC_PLAN];
		if(sensor.plan)
			timer.set_status(sensor.plan);

#elif __ROBOT_X400
		sensor.lbumper = (controller.receive_stream[22] & 0xf0)?true:false;
		sensor.rbumper = (controller.receive_stream[22] & 0x0f)?true:false;
		sensor.c_stub = (controller.receive_stream[24] << 16) | (controller.receive_stream[25] << 8) | controller.receive_stream[26];
		sensor.key = controller.receive_stream[27];
		sensor.c_s = controller.receive_stream[28];
		sensor.w_tank_ = (controller.receive_stream[29] > 0) ? true : false;
		sensor.batv = controller.receive_stream[30];

		sensor.lcliff = ((controller.receive_stream[31] << 8) | controller.receive_stream[32]);
		sensor.fcliff = ((controller.receive_stream[33] << 8) | controller.receive_stream[34]);
		sensor.rcliff = ((controller.receive_stream[35] << 8) | controller.receive_stream[36]);

		sensor.lbrush_oc_ = (controller.receive_stream[37] & 0x08) ? true : false;		// left brush over current
		sensor.mbrush_oc_ = (controller.receive_stream[37] & 0x04) ? true : false;		// main brush over current
		sensor.rbrush_oc_ = (controller.receive_stream[37] & 0x02) ? true : false;		// right brush over current
		sensor.vcum_oc = (controller.receive_stream[37] & 0x01) ? true : false;		// vaccum over current
		sensor.gyro_dymc_ = controller.receive_stream[38];
		sensor.right_wall_ = ((controller.receive_stream[39]<<8)|controller.receive_stream[40]);
		sensor.x_acc = (controller.receive_stream[41]<<8)|controller.receive_stream[42]; //in mG
		sensor.y_acc = (controller.receive_stream[43]<<8)|controller.receive_stream[44];//in mG
		sensor.z_acc = (controller.receive_stream[45]<<8)|controller.receive_stream[46]; //in mG
#endif
...
void *serial_send_routine(void*)
{
	...
	while(send_stream_thread){
		...
		if(controller.getSendData(CTL_OMNI_RESET) & 0x01)
			omni.clear();
	}
...
void process_beep()
{
	...
	if (temp_speaker_silence_time_count == 0){
		...
		controller.setSendData(CTL_BUZZER, robotbase_sound_code & 0xFF);
	}
