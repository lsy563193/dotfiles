...
void MoveType::update(const Cell_t& curr, PPTargetType& path) {
	...
	else if (cm_is_navigation())
	{
		if (should_follow_wall(g_old_dir, curr, path))
		{
			...
			int move_type_tmp = (g_old_dir == POS_X ^ delta_y > 0) ? MT_FOLLOW_LEFT_WALL : MT_FOLLOW_RIGHT_WALL;
			set(move_type_tmp);
...
bool MoveType::should_follow_wall(const int16_t dir, const Cell_t& curr, PPTargetType& path)
{
	bool ret = true;
	auto delta_y = path.back().Y - curr.Y;
