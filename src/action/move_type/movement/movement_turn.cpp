...
//	auto rad_diff = getPosition().th - slam_target + odom.getRadian();
	...
	ROS_WARN("target_radian_(%lf) = ranged_radian(slam_target(%lf)  - getPosition().th(%lf) + odom.getRadian(%lf))",
	...
	ROS_WARN("%s, %d: MovementTurn init, target_radian_: \033[32m%lf (in degree)\033[0m, current slam_target: \033[32m%lf (in degree)\033[0m, timeout:(%.2f)s."
