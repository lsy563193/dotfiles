 ...
 * */
	...
	if (distance > SEARCH_BEFORE_DIS && getNextMode() != cm_spot) {// closed
		...
		ROS_ERROR("distance > %d,limit %d, closed %d",SEARCH_BEFORE_DIS,closed_count_limit_,is_closed);
		return true;
...
void ACleanMode::switchInStateExploration() {
	...
	old_dir_ = iterate_point_->dir;
	Cells cells{};
	auto is_found = clean_path_algorithm_->dijkstra(clean_map_, getPosition().toCell(), cells,true,  CellEqual(Cell_t{0, 0}),
													isAccessable(clean_map_.genRange(), &clean_map_));
	if (!is_found) {
...
bool ACleanMode::updateActionInStateFollowWall()
{
	...
	if(is_closed) {
		...
		if (closed_count_ >= closed_count_limit_ || isolate_count_ >= isolate_count_limit_) {
			ROS_INFO("closed_count_(%d),closed_count_limit_(%d),isolate_count_(%d),isolate_count_limit_(%d)",
							 closed_count_, closed_count_limit_, isolate_count_, isolate_count_limit_);
...
bool ACleanMode::isIsolate(const Cell_t& curr) {
	...
	auto cells = Cells{};
//	auto is_found = fw_tmp_map.dijkstra(curr, cells,[&](const Cell_t& c_it){return c_it == external_target;},true, true);

	auto expand_condition = [&](const Cell_t cell, const Cell_t neighbor_cell){
		return fw_tmp_map.isAccessibleNeighbor(neighbor_cell);
	};

	bool is_found = clean_path_algorithm_->dijkstra(fw_tmp_map, curr, cells, false,
