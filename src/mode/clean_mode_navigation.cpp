...
bool CleanModeNav::setNextState()
{
	...
	while (ros::ok() && !state_confirm)
	{
		...
		else if(state_i_ == st_clean)
		{
			...
			{
				if (clean_path_algorithm_->checkTrapped(*map_, getPosition().toCell()))
				{
					// Robot trapped.
					state_i_ = st_trapped;
					stateInit(state_i_);
		...
		else if (state_i_ == st_trapped)
		{
			...
			else if (!clean_path_algorithm_->checkTrapped(*map_, getPosition().toCell()))
			{
		...
		else if (state_i_ == st_resume_low_battery_charge)
		{
			...
			else
			{
				old_dir_ = new_dir_;
				ROS_ERROR("old_dir_(%d)", old_dir_);
				clean_path_algorithm_->generateShortestPath(*map_, getPosition(), continue_point_, old_dir_, plan_path_);
				if (!plan_path_.empty())
				{
					new_dir_ = (MapDirection)plan_path_.front().th;
					ROS_ERROR("new_dir_(%d)", new_dir_);
...
bool CleanModeNav::isOverOriginLine()
{
		...
		|| (p_mt->target_point_.y < p_mt->start_point_.y && (curr.y - p_mt->start_point_.y) > 120))
	{
		...
		else if (map_->isBlockCleaned(curr.toCell().x, curr.toCell().y)) // If robot covers a big block, stop.
		{
...
//					 countToCell(s_curr_p.y));
	...
	else if (is_pos_dir ^ s_curr_p.y < p_mt->target_point_.y)
	{
		...
		if (map_->isBlocksAtY(s_curr_p.toCell().x + dx, s_curr_p.toCell().y)) {
...
//		ROS_INFO("%s %d: passed_path_.back(%d %d)", __FUNCTION__, __LINE__, passed_path_.back().x, passed_path_.back().y);
		return !map_->isBlockAccessible(passed_path_.back().x, passed_path_.back().y);
	}
