...
public:
	...
	virtual void optimizePath(GridMap &map, Cells &path, const Dir_t& pri_dir);
...
private:
	...
	void optimizePath(GridMap &map, Cells &path, const Dir_t& pri_dir) override ;
...
public:
	explicit GoHomeWay_t(std::string name, const func_compare_two_t& compare,bool is_allow_update_map=false, bool is_clear_block=false)
			:expand_condition(compare),is_allow_update_map_(is_allow_update_map),is_clear_block_(is_clear_block) { };

	std::unique_ptr<GridMap> updateMap(GridMap &map, const Point_t &curr);
	void clearBlock(GridMap &map);
	bool isClearBlock() {return is_clear_block_;};
	bool displayName() {std::cout << name_ <<std::endl;};
//private:
...
{
public:
	GoHomePathAlgorithm(GridMap& map, HomePoints_t* p_home_points, HomePoints_t* p_start_points);
